...
return (
    ...
    <CommentMenu
        isOpen={isDrawerOpen}
        setIsOpen={(open) => { setIsDrawerOpen(open); if (!open) setReplyTarget(null) }}
        postId={id}
        parentCommentId={replyTarget}
        onNewComment={handleNewComment}
    />
    <h3>Respuestas</h3>
    <input type="text" placeholder="Buscar por usuario..." value={searchName} onChange={(e) => setSearchName(e.target.value)} className="input"/>
    {loadingComment ? <p>Cargando comentarios...</p> : commentFilter.length > 0 ? commentFilter.map(c => (
        ...
        {usuarioLog && <button onClick={() => btnLike(c.id, usuarioLog.id)}> { (c.likedUsers || []).includes(usuarioLog.id) ? 'Quitar Like' : 'Like' } {c.likes}</button>}
        </div>
